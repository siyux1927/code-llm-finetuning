...
**这些风险现在不只是写在这里** —— `docs/grilling_m6_pre.md` Q6 把每一条都配了一个轻量冒烟测试（小模型跑一遍同样的 API 路径，几秒钟验证库版本/显存/格式兼容性），并且直接写进了 `notebooks/run_quantization_colab.ipynb` 里对应步骤前面，不用等 7B 模型跑到一半才发现问题。
